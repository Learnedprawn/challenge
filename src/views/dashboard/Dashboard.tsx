...
const Dashboard = () => {
  ...
  const [contractTimestamp, setContractTimestamp] = useState();
  ...
  async function etherscanAPI() {
    try {
      const res = await axios.get(`http://localhost:3005/etherscan`);
      const result = res.data.data.result[0]
      console.log("res", result);
      setContractAddress(result.contractAddress)
      setContractCreator(result.contractCreator)
      setContractTimestamp(result.timestamp)
      setContractData(JSON.stringify(result));
    } catch (err) {
      console.log(err);
    }
  }
    ...
    <Box>
      ...
      >
        ...
        <Box sx={{ overflow: "auto", width: { sm: "100%" } }}>
          ...
          <Box sx={{ padding: "80px 24px" }}>
            Assignment:
            <Button onClick={getName}>Name</Button>
            <Box sx={{ padding: "80px 24px" }}>Name: {name}</Box>
            <Button onClick={getSymbol}>Symbol</Button>
            <Box sx={{ padding: "80px 24px" }}>Symbol: {symbol}</Box>
            <Button onClick={etherscanAPI}>Details</Button>
            <Box sx={{ padding: "80px 24px" }}>
              Contract Address: {contractData?  contractAddress : "Get details"} 
              <br></br>
              Contract Creator Address: {contractData?  contractCreator: ""}
              <br></br>
              Contract Timestamp: {contractData?  contractTimestamp: ""}
            </Box>
          </Box>
        </Box>
      </Box>
